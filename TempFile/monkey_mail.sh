...
android_version="Android `adb -s ${device_id} shell getprop ro.build.version.release`"
device=`adb -s ${device_id} shell getprop ro.product.model`
#adb shell dumpsys window displays | grep "init"
#display=`adb -s ${device_id} shell dumpsys window |grep Display |head -n 1 |cut -f 2 -d =| awk '{print $1}'`
